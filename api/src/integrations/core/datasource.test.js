const CoreDataSource = require('./datasource');

describe('CoreDataSource', () => {
  describe('constructor', () => {
    it('sets params', () => {
      const dataSource = new CoreDataSource(null, null, { params: 1 });
      expect(dataSource.params).toEqual({ params: 1 });
    });

    it('sets default params if not provided', () => {
      const dataSource = new CoreDataSource();
      expect(dataSource.params).toEqual({});
    });

    it('sets auth_payload', () => {
      const dataSource = new CoreDataSource(null, { auth_payload: 'some payload' }, { params: 1 });
  ...
  describe('updateRecordType', () => {
    ...
    it('returns early if no epic records are found', async () => {
      ...
      // mock CoreDataSource functions called by updateRecordType()
    ...
    it('calls all functions necessary to update epic records for the given type', async () => {
      ...
      const dataSource = new CoreDataSource({ updateTaskRecord: jest.fn() }, null, { param1: 1 });
